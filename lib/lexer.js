exports.receive = function lexer(code, shouldReturnTemps) {
  ...
  return html.compile(code, shouldReturnTemps);
  ...
  function compress(code) { //removes double tabbing
    ...
    code.forEach(function (line) { //we want to remove double tabbing but not in markdown or coffeescript.
      ...
      if (line !== line.trim()) {
        curTab = (line.split(line.trim())[0].match(/\t/g) || []).length;
      } else {
        curTab = 0;
      }
  ...
  function expand(code) { //deals with - operator.
    ...
    code.forEach(function (line) {
      ...
      preTab = curTab;         //update tab variables
      if (line !== line.trim()) {
        var curTab = (line.split(line.trim())[0].match(/\t/g) || []).length;
      } else {
        curTab = 0;
      }
      tabDiff = curTab - preTab;
